...
<div class="main">
    <header class="topbar">
        <div class="topbar-left">
            <h1>Modul Kasir</h1>
            <p class="breadcrumb" style="margin:0; font-size:12px; color:var(--gray-600);">Front Office &rsaquo; <span>Kasir</span></p>
        </div>
        <div class="topbar-right" style="display:flex; align-items:center; gap:16px;">
            <?php 
            $notif_emr_cnt = !empty($pasien_selesai_emr) ? count($pasien_selesai_emr) : 0;
            ?>
            <button class="icon-btn" title="<?= $notif_emr_cnt > 0 ? $notif_emr_cnt . ' Pasien Selesai EMR Menunggu Kasir' : 'Tidak ada notifikasi baru' ?>" onclick="if(<?= $notif_emr_cnt ?> > 0) window.location.href='?page=kasir'" style="background:none; border:none; position:relative; cursor:pointer; font-size:18px; color:var(--gray-600);">
                <i class="fa-solid fa-bell <?= $notif_emr_cnt > 0 ? 'fa-shake' : '' ?>" style="<?= $notif_emr_cnt > 0 ? 'color: #2563eb;' : '' ?>"></i>
                <?php if ($notif_emr_cnt > 0): ?>
                <span class="badge" style="background: #ef4444; color: white; font-weight: bold; position:absolute; top:-6px; right:-8px; padding:2px 6px; border-radius:10px; font-size:10px;"><?= $notif_emr_cnt ?></span>
                <?php else: ?>
                <span class="badge" style="background: var(--gray-400); color: white; position:absolute; top:-6px; right:-8px; padding:2px 6px; border-radius:10px; font-size:10px;">0</span>
                <?php endif; ?>
            </button>
            <div class="user-info" style="display:flex; align-items:center; gap:10px;">
                <div class="user-avatar" style="width:36px; height:36px; border-radius:50%; background:#e0f2fe; color:#0369a1; display:flex; align-items:center; justify-content:center;"><i class="fa-solid fa-user-tie"></i></div>
                <div class="user-text" style="line-height:1.2;">
                    <strong style="display:block; font-size:13px; color:var(--gray-800);">Dr. Hendrawan</strong>
                    <small style="font-size:11px; color:var(--gray-600);">Front Office Supervisor</small>
                </div>
            </div>
        </div>
    </header>
